...
Original file is located at
    https://colab.research.google.com/drive/1ZF9DzBSSdivuWY0_jov0Ei2P0TUzhdPd
...
class SoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = gradOutput * self.output - self.output* np.sum(gradOutput * self.output, axis=1).T[:, np.newaxis]
        return self.gradInput
...
class BatchNormalization(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = (gradOutput / np.sqrt(np.var(input, axis=0) + self.EPS)) - np.sum(gradOutput / np.sqrt(np.var(input, axis=0) + self.EPS), axis = 0) / input.shape[0]
        self.gradInput -= (input - np.mean(input, axis=0)) / input.shape[0] * np.sum(gradOutput * (input - np.mean(input, axis=0)) / np.sqrt(np.var(input, axis=0) + self.EPS) / (np.var(input, axis=0) + self.EPS), axis=0)  
        return self.gradInput
...
class Dropout(Module):
    ...
    def updateOutput(self, input):
        # Your code goes here. ################################################
        if self.training:
            self.mask = bernoulli.rvs(1 - self.p, size=input.shape) / (1 - self.p)
        else:
...
class LeakyReLU(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = gradOutput * (np.multiply(input < 0, self.slope) + np.multiply(input >= 0, 1))
        return self.gradInput
...
class ELU(Module):
    ...
    def updateOutput(self, input):
        # Your code goes here. ################################################
        self.output = np.maximum(input, 0) + (np.exp(np.minimum(input, 0)) - 1) * self.alpha
        return  self.output
    
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = gradOutput * (np.multiply(input > 0, 1) + np.multiply(input <= 0, self.alpha * np.exp(input)))
        return self.gradInput
...
class SoftPlus(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = sigmoid(input) * gradOutput
        return self.gradInput
...
class ClassNLLCriterion(Criterion):
    ...
    def updateOutput(self, input, target): 
        # Your code goes here. ################################################
        self.output = -np.sum(input * target) / target.shape[0]
        return self.output
